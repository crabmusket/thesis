...
The factor $\dot{m}_i$, defined below, refers to the amount of water entering node $i$ from node $i+1$, the node above it, and is therefore positive when water is flowing downwards through the tank (in the direction of drawing).
\begin{equation}
...
\begin{equation}
   ...
   \Q{inlet} = \B{col} \dot{m}_c (T_c - T_i)
             ...
             + \Q{aux}
\end{equation}
Note that special cases are necessary for the end nodes, as functions on indices $N$ and $-1$ are not defined.

In \autoref{eq:Q-inlet} we introduced the two \emph{control functions} $\B{col}$ and $\B{load}$ defined by \authors{Cristofari02}, which are binary indicator functions that determine which nodes receive hot and cold water flows from the inlets.
...
\begin{eqnarray}
   ...
   \end{dcases*}
\end{eqnarray}
The mass flow through each node is defined by \authors{Cristofari02} using sums of these collector functions to express the presence of an inlet above/below the current node being examined.
\begin{equation}
