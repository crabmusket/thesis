...
\begin{equation}
	\label{eq:node-dT}
	\rho_i C_i v_i \dot{T}_i = U_{amb} + U_{input} + u_{mflow}
\end{equation}
...
\begin{equation}
	U_{input} = B^c_i \dot{m}_c (T_c - T_i) + B^l_i \dot{m}_l (T_l - T_i)
\end{equation}

\begin{equation}
	U_{mflow} = \dot{m}_i C_i (T_{i-1} - T_i) (\dot{m}_i > 0) + \dot{m}_{i+1} C_i (T_i - T_{i+1}) (\dot{m}_{i+1} < 0)
\end{equation}

\begin{equation}
	\dot{m}_i = \dot{m}_l \sum_{j=1}^{i-1} B^l_j - \dot{m}_l \sum_{j=i+1}^{N-1} B^c_j
\end{equation}
